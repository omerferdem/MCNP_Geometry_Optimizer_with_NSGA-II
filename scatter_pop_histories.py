...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Usage: python scatter_pop_histories.py <x_len> <y_len> <plotted_objectives>")
        sys.exit(1)
    try:
...
df = pd.read_csv('population_history.csv')
...
plt.title('Complete Populations with Generations')

# Save the plot to a PNG file with 200 DPI
plt.savefig('historied_generations.png', dpi=200)
